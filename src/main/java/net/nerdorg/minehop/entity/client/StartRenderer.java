...
public class StartRenderer extends MobEntityRenderer<StartEntity, StartModel> {
    ...
    public void render(StartEntity startEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        ...
        float time = (((float) System.nanoTime() - (float) MinehopAddon.startTime) / 1000000000f);

        if (MinehopAddon.startTime != 0) {
        ...
        if (corner1 != null && corner2 != null) {
            ...
            if (!client.player.isCreative() && !client.player.isSpectator() && (MinehopAddon.groundedList.contains(client.player.getNameForScoreboard()))) {
                if (colliderBox.contains(client.player.getPos())) {
                    MinehopAddon.startTime = System.nanoTime();
                    MinehopAddon.lastSendTime = 0;
                }
            }
