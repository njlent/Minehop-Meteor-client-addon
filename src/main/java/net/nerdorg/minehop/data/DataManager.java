...
public class DataManager {
    ...
    public static void register() {
        ServerWorldEvents.LOAD.register(((server, world) -> {
            MinehopAddon.mapList = new ArrayList<>();
            MinehopAddon.recordList = new ArrayList<>();
            MinehopAddon.personalRecordList = new ArrayList<>();
            ...
            if (newMapList != null) {
                MinehopAddon.mapList = newMapList;
            }
            if (newPersonalRecordList != null) {
                MinehopAddon.personalRecordList = newPersonalRecordList;
            }
            if (newRecordList != null) {
                MinehopAddon.recordList = newRecordList;
            }
        }));

        ServerWorldEvents.UNLOAD.register(((server, world) -> {
            DataManager.saveData(world, mapListLocation, MinehopAddon.mapList);
            DataManager.saveData(world, pbListLocation, MinehopAddon.personalRecordList);
            DataManager.saveData(world, recordsListLocation, MinehopAddon.recordList);
        }));
    }

    public static RecordData removePersonalRecordsForPlayer(String mapName, String playerName) {
        if (MinehopAddon.personalRecordList != null) {
            for (RecordData recordData : MinehopAddon.personalRecordList) {
                if (recordData.map_name.equals(mapName) && recordData.name.equals(playerName)) {
                    MinehopAddon.personalRecordList.remove(recordData);
                    return recordData;
                }
            }
        }
    ...
    public static RecordData removeRecordsForPlayer(String mapName, String playerName) {
        if (MinehopAddon.recordList != null) {
            for (RecordData recordData : MinehopAddon.recordList) {
                if (recordData.map_name.equals(mapName) && recordData.name.equals(playerName)) {
                    MinehopAddon.recordList.remove(recordData);
                    double lowestTime = Double.MAX_VALUE;
                    RecordData recordToAdd = null;
                    for (RecordData newRecordData : MinehopAddon.recordList) {
                    ...
                    if (recordToAdd != null) {
                        MinehopAddon.recordList.add(recordToAdd);
                    }
                    return recordData;
                }
            }
    ...
    public static MapData getMap(String mapName) {
        if (MinehopAddon.mapList != null) {
            for (MapData mapData : MinehopAddon.mapList) {
