...
public class InstagibItem extends Item {
    ...
    private EntityHitResult raycastEntities(ServerPlayerEntity player, Vec3d startPos, Vec3d endPos, double maxDistance, ServerWorld world) {
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        if (world instanceof ServerWorld serverWorld) {
            ...
            if (!gibDelayList.containsKey(user.getNameForScoreboard()) || serverWorld.getServer().getTicks() > gibDelayList.get(user.getNameForScoreboard()) + 15) {
                ...
                serverWorld.playSound(null, serverPlayerEntity.getX(), serverPlayerEntity.getY(), serverPlayerEntity.getZ(), SoundEvents.ENTITY_WARDEN_ATTACK_IMPACT, SoundCategory.PLAYERS, 1f, 1f);
                serverWorld.playSound(user, user.getBlockPos(), SoundEvents.ENTITY_WARDEN_ATTACK_IMPACT, SoundCategory.PLAYERS, 1f, 1f);

                EntityHitResult entityHitResult = raycastEntities(serverPlayerEntity, startPos, endPos, 64, serverWorld);
                if (entityHitResult != null) {
                    endPos = entityHitResult.getPos();
                    Entity hitEntity = entityHitResult.getEntity();
                    serverWorld.playSound(null, serverPlayerEntity.getX(), serverPlayerEntity.getY(), serverPlayerEntity.getZ(), SoundEvents.ENTITY_PLAYER_HURT, SoundCategory.PLAYERS, 1f, 1f);
                    handleInstaGibHit(serverPlayerEntity, hitEntity, serverWorld);
    ...
    private void handleInstaGibHit(ServerPlayerEntity attacker, Entity target, ServerWorld attackerWorld) {
        ...
        if (mapName != null) {
            ...
            if (mapData != null) {
                ...
                for (ServerWorld serverWorld : attackerWorld.getServer().getWorlds()) {
                ...
                if (foundWorld != null) {
                    ...
                    target.teleport(foundWorld, targetPos.getX(), targetPos.getY(), targetPos.getZ(), Set.of(), (float) rotPos.getY(), (float) rotPos.getX(), true);
                    if (target instanceof ServerPlayerEntity targetPlayerEntity) {
                        ...
                        foundWorld.playSound(null, targetPlayerEntity.getX(), targetPlayerEntity.getY(), targetPlayerEntity.getZ(), SoundEvents.ENTITY_ITEM_BREAK, SoundCategory.PLAYERS, 1f, 1f);
                    }
                }
            }
        }
