...
public class MapUtilCommands {
            ...
            .then(LiteralArgumentBuilder.<ServerCommandSource>literal("top")
                .then(RequiredArgumentBuilder.<ServerCommandSource, String>argument("map_name", StringArgumentType.string())
                    .suggests((context, builder) -> {
                        for (DataManager.MapData mapData : MinehopAddon.mapList) {
    ...
    private static void handleAddCheckpoint(CommandContext<ServerCommandSource> context) {
        ...
        for (Object object : MinehopAddon.mapList) {
        ...
        if (mapToAddTo != null) {
            ...
            MinehopAddon.mapList.remove(mapToAddTo);
            mapToAddTo.checkpointPositions.add(new ArrayList<>(Arrays.asList(new Vec3d(serverPlayerEntity.getX(), serverPlayerEntity.getY(), serverPlayerEntity.getZ()), new Vec3d(serverPlayerEntity.getRotationClient().x, serverPlayerEntity.getRotationClient().y, 0))));
            MinehopAddon.mapList.add(mapToAddTo);
            DataManager.saveData(context.getSource().getWorld(), DataManager.mapListLocation, MinehopAddon.mapList);
        }
        else {
    ...
    private static void handleRestart(CommandContext<ServerCommandSource> context) {
        ...
        Zone closestEntity = MinehopAddon.playerMapLocation.get(serverPlayerEntity.getUuidAsString());
        ...
        if (closestEntity == null) {
            ...
            if (currentMapData != null) {
                if (currentMapData.worldKey == null || currentMapData.worldKey.equals("")) {
                    MinehopAddon.mapList.remove(currentMapData);
                    currentMapData.worldKey = context.getSource().getServer().getOverworld().getRegistryKey().toString();
                    MinehopAddon.mapList.add(currentMapData);
                    DataManager.saveData(context.getSource().getWorld(), DataManager.mapListLocation, MinehopAddon.mapList);
                }
                MinehopAddon.timerManager.remove(serverPlayerEntity.getNameForScoreboard());
                ...
                if (foundWorld != null) {
                    if (!serverPlayerEntity.isSpectator()) {
                        ...
                        serverPlayerEntity.teleport(foundWorld, currentMapData.x, currentMapData.y, currentMapData.z, Set.of(), (float) currentMapData.yrot, (float) currentMapData.xrot, true);
                        if (SpectateCommands.spectatorList.containsKey(serverPlayerEntity.getNameForScoreboard())) {
                            ...
                            for (String spectator : spectators) {
                                if (!spectator.equals(serverPlayerEntity.getNameForScoreboard())) {
                                    ...
                                    spectatorPlayer.teleport(foundWorld, serverPlayerEntity.getX(), serverPlayerEntity.getY(), serverPlayerEntity.getZ(), Set.of(), serverPlayerEntity.getYaw(), serverPlayerEntity.getPitch(), true);
        ...
        if (tpData != null) {
            ...
            if (foundWorld != null) {
                if (tpData.hns || tpData.arena) {
                    ...
                    MinehopAddon.playerMapLocation.put(serverPlayerEntity.getUuidAsString(), gamemodeEntity);
                }
                ...
                if (!serverPlayerEntity.isSpectator()) {
                    ...
                    serverPlayerEntity.teleport(foundWorld, targetPos.getX(), targetPos.getY(), targetPos.getZ(), Set.of(), (float) rotPos.getY(), (float) rotPos.getX(), true);
                    if (SpectateCommands.spectatorList.containsKey(serverPlayerEntity.getNameForScoreboard())) {
                        List<String> spectators = SpectateCommands.spectatorList.get(serverPlayerEntity.getNameForScoreboard());
                        for (String spectator : spectators) {
                            if (!spectator.equals(serverPlayerEntity.getNameForScoreboard())) {
    ...
    private static void handleAdd(CommandContext<ServerCommandSource> context) {
        ...
        MinehopAddon.mapList.add(mapData);
        DataManager.saveData(context.getSource().getWorld(), DataManager.mapListLocation, MinehopAddon.mapList);
    ...
    private static void handleInvalidatePlayer(CommandContext<ServerCommandSource> context) {
        ...
        if (invalidateData != null) {
            if (DataManager.getPersonalRecord(playerName, invalidateData.name) != null) {
                ...
                DataManager.saveData(context.getSource().getWorld(), DataManager.pbListLocation, MinehopAddon.personalRecordList);

                DataManager.removeRecordsForPlayer(name, playerName);
                DataManager.saveData(context.getSource().getWorld(), DataManager.recordsListLocation, MinehopAddon.recordList);
    ...
    private static void handleRemove(CommandContext<ServerCommandSource> context) {
        ...
        for (Object object : MinehopAddon.mapList) {
            if (object instanceof DataManager.MapData mapData) {
                if (mapData.name.equals(name)) {
                    removedData = mapData;
                    MinehopAddon.mapList.remove(mapData);
                    DataManager.saveData(context.getSource().getWorld(), DataManager.mapListLocation, MinehopAddon.mapList);
                    break;
                }
            }
    ...
    private static void handleSetDifficulty(CommandContext<ServerCommandSource> context) {
        ...
        if (difficultyData != null) {
            ...
            MinehopAddon.mapList.add(difficultyData);
            DataManager.saveData(context.getSource().getWorld(), DataManager.mapListLocation, MinehopAddon.mapList);
    ...
    private static void handleSetMapSpawn(CommandContext<ServerCommandSource> context) {
        ...
        if (spawnData != null) {
            ...
            MinehopAddon.mapList.add(spawnData);
            DataManager.saveData(context.getSource().getWorld(), DataManager.mapListLocation, MinehopAddon.mapList);
    ...
    private static void handleToggleArena(CommandContext<ServerCommandSource> context) {
        ...
        if (toggleData != null) {
            ...
            MinehopAddon.mapList.add(toggleData);
            DataManager.saveData(context.getSource().getWorld(), DataManager.mapListLocation, MinehopAddon.mapList);
    ...
    private static void handleListTop(CommandContext<ServerCommandSource> context) {
        ...
        for (DataManager.RecordData recordData : MinehopAddon.personalRecordList) {
