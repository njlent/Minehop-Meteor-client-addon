...
public class PacketHandler {
    ...
    public static void sendConfigToClient(ServerPlayerEntity player, MinehopConfig config) {
        // TODO: Fix networking API for 1.21.10
        ...
        // buf.writeDouble(MinehopAddon.speedCapMap.containsKey(player.getNameForScoreboard()) ? MinehopAddon.speedCapMap.get(player.getNameForScoreboard()) : 1000000);
    ...
    private static void handleMapCompletion(ServerPlayerEntity player, MinecraftServer server, String map_name, float time) {
        ...
        if (!player.isCreative() && !player.isSpectator()) {
            if (MinehopAddon.timerManager.containsKey(player.getNameForScoreboard())) {
                HashMap<String, Long> timerMap = MinehopAddon.timerManager.get(player.getNameForScoreboard());
                ...
                if (time < rawTime + (ping_limit / 1000f) && time > rawTime - (ping_limit / 1000f)) {
                    ...
                    if (mapRecord != null) {
                        if (time < mapRecord.time) {
                            ...
                            MinehopAddon.recordList.remove(mapRecord);
                            ...
                            MinehopAddon.recordList.add(new DataManager.RecordData(player.getNameForScoreboard(), map_name, time));
                            ServerWorld serverWorld = server.getOverworld();
                            DataManager.saveData(serverWorld, DataManager.recordsListLocation, MinehopAddon.recordList);
                            ...
                            ReplayManager.saveRecordReplay(serverWorld, replay);
                    ...
                    if (mapPersonalRecord != null) {
                        if (time < mapPersonalRecord.time) {
                            ...
                            MinehopAddon.personalRecordList.remove(mapPersonalRecord);
                            MinehopAddon.personalRecordList.add(new DataManager.RecordData(player.getNameForScoreboard(), map_name, time));
                            ServerWorld pbServerWorld = server.getOverworld();
                            DataManager.saveData(pbServerWorld, DataManager.pbListLocation, MinehopAddon.personalRecordList);
                        }
                    } else {
    ...
    public static void sendMaps(ServerPlayerEntity player) {
        // TODO: Fix networking API for 1.21.10
        // PacketByteBuf buf = new PacketByteBuf(Unpooled.buffer());
        // buf.writeInt(MinehopAddon.mapList.size());
    ...
    public static void sendPersonalRecords(ServerPlayerEntity player) {
        // TODO: Fix networking API for 1.21.10
        // PacketByteBuf buf = new PacketByteBuf(Unpooled.buffer());
        // buf.writeInt(MinehopAddon.personalRecordList.size());
    ...
    public static void registerReceivers() {
        // TODO: Fix networking API for 1.21.10 - registerGlobalReceiver now requires CustomPayload
        ...
        //         if (player != null && MinehopAddon.timerManager.containsKey(player.getNameForScoreboard())) {
        //             HashMap<String, Long> timerMap = MinehopAddon.timerManager.get(player.getNameForScoreboard());
        //             List<String> keyList = timerMap.keySet().stream().toList();
        ...
        //
        //     MinehopAddon.lastEfficiencyMap.put(player.getNameForScoreboard(), new ReplayManager.SSJEntry(jump_count, last_jump_speed, last_efficiency));
        //
        //     if (SpectateCommands.spectatorList.containsKey(player.getNameForScoreboard())) {
        //         List<String> spectators = SpectateCommands.spectatorList.get(player.getNameForScoreboard());
        //         for (String spectator : spectators) {
