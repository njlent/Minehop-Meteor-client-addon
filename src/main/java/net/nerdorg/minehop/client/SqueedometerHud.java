...
public class SqueedometerHud {
    ...
    public void drawJHUD(DrawContext context, MinehopConfig config) {
        ...
        if (client != null) {
            Vec3d playerPosVec = client.player.getPos();
            if (MinehopAddon.jump_count > 0) {
                this.client = MinecraftClient.getInstance();
                this.textRenderer = client.textRenderer;

                double effPercent;
                if (client.player.isSpectator()) {
                    effPercent = MinehopAddon.last_efficiency;
                }
                else {
                ...
                String ssjText = SpeedCalculator.ssjText(MinehopAddon.last_jump_speed, MinehopAddon.jump_count);
                ...
                if (Minehop.gaugeListMap.containsKey(client.player.getNameForScoreboard())) {
                    if (client.world.getTime() % 4 == 0) {
                        ...
                        MinehopAddon.gauge = gaugeList.stream().mapToDouble(Double::doubleValue).average().orElse(Double.NaN);

                        Minehop.gaugeListMap.put(client.player.getNameForScoreboard(), gaugeList);
                    }
                }

                Pair<String, Integer> gaugeData = SpeedCalculator.gaugeText(MinehopAddon.gauge, config.jHud.gaugeHud.horizontal_gauge);
                ...
                int gaugeColor = getGaugeColor(MinehopAddon.gauge);
            ...
            if (config.jHud.prespeedHud.show_prespeed) {
                String preText = SpeedCalculator.speedText(MinehopAddon.start_jump_speed);

                double travelledX = playerPosVec.x - client.player.prevX;
                double travelledZ = playerPosVec.z - client.player.prevZ;
                double speed = (double) MathHelper.sqrt((float) (travelledX * travelledX + travelledZ * travelledZ));
                if (MinehopAddon.wasOnGround && !client.player.isOnGround() && MinehopAddon.jump_count == 0) {
                    MinehopAddon.start_jump_speed = speed;
                }
            ...
            if (client.player == null || !client.player.isSpectator()) {
                if (MinehopAddon.jumping) {
                    if (client.world.getTime() >= MinehopAddon.last_jump_time + 1 || client.world.getTime() < MinehopAddon.last_jump_time || MinehopAddon.last_jump_time == 0) {
                        if (client.player.isOnGround()) {
                            double travelledX = playerPosVec.x - client.player.prevX;
                            double travelledZ = playerPosVec.z - client.player.prevZ;
                            double speed = (double) MathHelper.sqrt((float) (travelledX * travelledX + travelledZ * travelledZ));
                            MinehopAddon.old_jump_speed = MinehopAddon.last_jump_speed;
                            MinehopAddon.last_jump_speed = speed;
                            MinehopAddon.jump_count += 1;
                            MinehopAddon.old_jump_time = MinehopAddon.last_jump_time;
                            MinehopAddon.last_jump_time = client.world.getTime();
                        }
                    }
                } else {
                    MinehopAddon.old_jump_speed = 0;
                    MinehopAddon.last_jump_speed = 0;
                    MinehopAddon.jump_count = 0;
                    MinehopAddon.old_jump_time = 0;
                    MinehopAddon.last_jump_time = 0;
                }
            }
            MinehopAddon.wasOnGround = client.player.isOnGround();
        }
    }
    ...
    public void drawSpectators(DrawContext context, float tickDelta) {
        ...
        for (int index = 0; index < MinehopAddon.spectatorList.size(); index++) {
            top += this.textRenderer.fontHeight * 2;
            context.drawTextWithShadow(this.textRenderer, MinehopAddon.spectatorList.get(index), left, top, Formatting.RED.getColorValue());
        }
    }
}
